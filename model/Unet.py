...
class UNet(nn.Module):
    ...
    def forward(self, x, label, timestep, ctx_mask=None):
        ...
        label = fun.one_hot(label, num_classes = self.num_cls).type(torch.float)
        ...
        c_emb1 = self.ctx_emb1(label).view(-1, self.out_c * 2, 1, 1)
        c_emb2 = self.ctx_emb2(label).view(-1, self.out_c, 1, 1)
        t_emb1 = self.time_emb1(timestep).view(-1, self.out_c * 2, 1, 1)
        t_emb2 = self.time_emb2(timestep).view(-1, self.out_c, 1, 1)
